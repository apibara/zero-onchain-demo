...
const DataTable: React.FC<TableProps> = ({ data, onRowClick }) => {
  ...
  React.useEffect(() => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      for (let j = 0; j < data[i].length; j++) {
        const cell = cells[j];
        const changed = typeof data[i][j] !== 'string' ? false : prevDataRef.current[i]?.[j] !== data[i][j];
        if (cell && changed) {
    ...
    <table className={styles.root} ref={tableRef} onKeyDown={handleKeyDown}>
      <tbody className={styles.body}>
        {data.map((row, rowIndex) => (
          <tr key={rowIndex} className={styles.row} tabIndex={0} onClick={onRowClick ? () => onRowClick(row) : undefined}>
