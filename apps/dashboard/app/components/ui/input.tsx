...
import { findNextFocusable, classNames} from './utilities';
...
function Input({ caretChars, isBlink = true, label, placeholder, onChange, type, id, ...rest }: InputProps) {
  ...
  const containerClasses = classNames(styles.root, isFocused && styles.focused);
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className={styles.inputContainer}>
        <div className={classNames(styles.displayed, isPlaceholderVisible && styles.placeholder)}>
          {beforeCaretText}
          {!isPlaceholderVisible && <span className={classNames(styles.block, isBlink && styles.blink)}>{caretChars || ''}</span>}
